...
// ページ上の要素（Element)を参照
const elementCalcuLog = document.getElementById("calcuLog");
...
function update(calcType) {
  // １つ前の入力が数値
  if (wkBefore === "0") {
    elementCalcuLog.innerHTML = elementCalcuLog.innerHTML + Number(elementResult.innerHTML)+calcType; //計算ログ
    calculator();
  } 
  // １つ前の入力が演算子（演算子 入力しなおし）
    ...
    // 初回入力
    if (wkFirst === "1") {
      elementCalcuLog.innerHTML="0"+calcType; //計算ログ
    }
    else {
      // 演算子 入力しなおし
      let wkLogLastWord = elementCalcuLog.innerHTML.slice(-1); //ログ最後の１文字
      if (["+","-","*","/"].includes(wkLogLastWord)) {
        elementCalcuLog.innerHTML = elementCalcuLog.innerHTML.slice(0, -1) + calcType; //計算ログ　末尾1文字（前回の演算子）を削除
      }
      // イコールの後の演算子
      else{
        elementCalcuLog.innerHTML = elementCalcuLog.innerHTML + calcType; //計算ログ
      }
    }
  }
...
/** =がクリックされたときの処理 */
function dspResult(){
// 初回入力以外&前回数文字入力の時に処理を実行
if(wkFirst ==="0" && wkBefore ==="0"){
  elementCalcuLog.innerHTML=elementCalcuLog.innerHTML+Number(elementResult.innerHTML); //計算ログ
  calculator();
    wkCalc="=";//イコールsave
    wkBefore="1";
  } 
...
function clear(){
  ...
  elementCalcuLog.innerHTML="";//クリアする
  wkFirst ="1";
  wkTotal = 0;
  wkCalc ="+";
  wkBefore="1";
